...
const Safesellscomponent = ({user, s }) => {
const[product, setProduct] = useState({})
    const comfirm = async()=>{
       ...
       // ress.data && setComfirmPressed(true)
  ...
  return (
    ...
    <div className="flex justify-around  items-center">
        <button className="px-10 py-5 bg-[#ff4800] m-2 font-bold  text-white" onClick={comfirm}>Confirm  <sup className="border-2 border-white p-2 m-2">{new Date(s.createdAt).toDateString()}</sup></button>
    </div>
    </div> 
  )
}

export default Safesellscomponent
